...
export default function Courses({ dict }: { dict: Dictionary }) {
  return (
    <section id="courses" className="py-section" aria-labelledby="courses-title">
        ...
        <ul className="mt-14 grid gap-px border border-graphite-200 bg-graphite-200 sm:mt-20 sm:grid-cols-2 xl:grid-cols-3">
          {dict.courses.items.map((course, i) => (
            ...
            >
              <a
                href="#contact"
                className="group/card relative flex h-full flex-col justify-between gap-10 overflow-hidden p-8 transition-colors duration-500 hover:bg-ink sm:p-10"
              >
                {/* Sliding ink wash on hover */}
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute inset-0 -z-10 origin-bottom scale-y-0 bg-ink transition-transform duration-600 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover/card:scale-y-100"
                />

                <div>
                  <div className="flex items-baseline justify-between gap-4">
                    <span className="text-[0.6875rem] font-semibold uppercase tracking-[0.26em] text-graphite-400 transition-colors duration-500 group-hover/card:text-paper/50">
                      {course.code}
                    </span>
                    <span className="text-[0.6875rem] font-medium tracking-[0.02em] text-graphite-400 transition-colors duration-500 group-hover/card:text-paper/50">
                      {dict.courses.levels}
                    </span>
                  </div>

                  <h3 className="mt-6 text-[clamp(2rem,4.4vw,3.25rem)] font-bold leading-[0.95] tracking-[-0.04em] transition-colors duration-500 group-hover/card:text-paper">
                    {course.name}
                  </h3>

                  <p className="mt-5 text-sm leading-relaxed text-graphite-600 transition-colors duration-500 group-hover/card:text-paper/70">
                    {course.desc}
                  </p>
                  ...
                  <ul className="mt-7 flex flex-wrap gap-2">
                    {course.tags.map((tag) => (
                      <li
                        key={tag}
                        className="border border-graphite-200 px-3 py-1.5 text-[0.6875rem] font-medium tracking-[0.02em] text-graphite-500 transition-colors duration-500 group-hover/card:border-paper/25 group-hover/card:text-paper/65"
                      >
                        {tag}
                      </li>
                    ))}
                ...
                <span className="flex items-center gap-3 text-[0.8125rem] font-semibold tracking-[0.02em] transition-colors duration-500 group-hover/card:text-paper">
                  {dict.courses.more}
                  <span
                    aria-hidden="true"
           ...
           */}
          <Reveal as="li" delay={180} className="bg-ink text-paper">
            <a
              href="#contact"
            ...
            >
              <div>
                <span
                  aria-hidden="true"
                  className="inline-block size-1.5 bg-taxi"
                />
              ...
              <span className="flex items-center gap-3 text-[0.8125rem] font-semibold uppercase tracking-[0.14em]">
                {dict.nav.cta}
                <span
                  aria-hidden="true"
            ...
            <ul className="columns-1 gap-x-10 sm:columns-2 lg:col-span-7">
              {dict.courses.formats.map((format) => (
                ...
                >
                  <span
                    aria-hidden="true"
                    className="mt-[0.5rem] inline-block size-1 shrink-0 bg-ink"
                  />
                  {format}
                </li>
              ))}
          ...
          <div className="grid gap-8 lg:grid-cols-12 lg:gap-16">
            ...
            <ul className="grid grid-cols-2 gap-px self-start border border-graphite-200 bg-graphite-200 sm:grid-cols-3 lg:col-span-7">
              {EXAMS.map((exam) => (
                <li
                  key={exam}
                  className="flex items-center justify-center bg-paper px-4 py-7 text-center text-sm font-semibold tracking-[0.06em] transition-colors duration-400 hover:bg-ink hover:text-paper sm:py-9"
                >
                  {exam}
                </li>
              ))}
