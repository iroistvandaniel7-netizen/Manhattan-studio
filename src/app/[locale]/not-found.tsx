  ...
  return (
    <section className="py-section">
      <div className="container-x">
        <h1 className="text-[clamp(5rem,16vw,12rem)] font-extrabold leading-[0.85] tracking-[-0.028em] text-accent">
          404
        </h1>
        ...
        <ul className="mt-12 grid gap-10 border-t border-line pt-10 sm:grid-cols-3">
          {messages.map((message) => (
            <li key={message.locale} lang={message.locale}>
              <h2 className="text-lg font-bold tracking-[-0.012em]">{message.title}</h2>
